...
function my_church_register_theme_js_scripts() {
	...
	wp_localize_script('my-church-theme-script', 'cmsmasters_theme_script', $cmsmasters_localize_array);
}

add_action('wp_enqueue_scripts', 'my_church_register_theme_js_scripts');



/* Register Theme Navigations */
register_nav_menus(array( 
    'primary' => 	esc_html__('Primary Navigation', 'my-church'), 
    'footer' => 	esc_html__('Footer Navigation', 'my-church'), 
	'top_line' => 	esc_html__('Top Line Navigation', 'my-church') 
));
...
function my_church_fallback_menu($args) {
	$cmsmasters_option = my_church_get_global_options();
	...
	echo '<div class="navigation_wrap">' . "\n" . 
		'<ul id="navigation" class="' . 
			'navigation ' . 
			($cmsmasters_option['my-church' . '_header_styles'] == 'default' ? 'mid_nav' : 'bot_nav') . 
		'">' . "\n";
...
function my_church_single_project_layout($cmsmasters_layout) {
	if (
		is_singular('project') || 
		is_singular('profile') 
	) {
...
add_filter('cmsmasters_theme_page_layout_filter', 'my_church_single_project_layout');



/* Search Form */
function my_church_search_form($form) {
	$form = "<div class=\"search_bar_wrap\">
		<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<p class=\"search_field\">
				<input name=\"s\" placeholder=\"" . esc_attr__('Search...', 'my-church') . "\" value=\"\" type=\"search\" />
			</p>
...
add_filter('get_search_form', 'my_church_search_form');
