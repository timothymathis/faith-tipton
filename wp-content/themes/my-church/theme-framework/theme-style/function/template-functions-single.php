...
function my_church_prev_next_posts($order_cat = false) {
	$cmsmasters_post_type = get_post_type();
	
	$cmsmasters_post_type_name = get_post_type_object($cmsmasters_post_type)->labels->singular_name;
...
function my_church_sharing_box($title_box = false, $tag = 'h3') {
...
function my_church_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'my-church') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function my_church_related($tag = 'h3', $title = '', $no_title = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
...
function my_church_author_avatar($template_type = 'page') {
...
function my_church_get_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'my-church') . '</' . esc_html($tag) . '>' . "\n" .
