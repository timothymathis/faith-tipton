...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_puzzle ' . $cmsmasters_project_size); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		my_church_thumb_rollover(get_the_ID(), 'full', false, $rollover, false, false, false, false, false, false, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
						...
						'<div class="project_inner_table">' . 
							...
							if ($categories) {
								echo '<div class="cmsmasters_project_cont_info entry-meta">';
									...
									my_church_get_project_category(get_the_ID(), 'pj-categs', 'page', false);
		...
		if (!$title) {
			echo '<div class="dn">';
				my_church_project_heading(get_the_ID(), 'h3');
			echo '</div>';
		}
