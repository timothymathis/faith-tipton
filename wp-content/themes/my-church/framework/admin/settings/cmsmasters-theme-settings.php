...
add_action('admin_menu', 'my_church_add_menu');
add_action('admin_init', 'my_church_register_settings');
...
function my_church_get_settings() {
	$output = array();
	
	
	$page = my_church_get_admin_page();
	...
	case 'cmsmasters-settings':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'my-church' . CMSMASTERS_THEME_STYLE;
		$cmsmasters_settings_page_title = esc_html__('Theme General Settings', 'my-church');
		$cmsmasters_page_sections = my_church_options_general_sections();
		$cmsmasters_page_fields = my_church_options_general_fields();
		$cmsmasters_page_tabs = my_church_options_general_tabs();
	...
	case 'cmsmasters-settings' . '-font':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'my-church' . CMSMASTERS_THEME_STYLE . '_font';
		$cmsmasters_settings_page_title = esc_html__('Theme Fonts Settings', 'my-church');
		$cmsmasters_page_sections = my_church_options_font_sections();
		$cmsmasters_page_fields = my_church_options_font_fields();
		$cmsmasters_page_tabs = my_church_options_font_tabs();
	...
	case 'cmsmasters-settings' . '-color':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'my-church' . CMSMASTERS_THEME_STYLE . '_color';
		$cmsmasters_settings_page_title = esc_html__('Theme Color Schemes', 'my-church');
		$cmsmasters_page_sections = my_church_options_color_sections();
		$cmsmasters_page_fields = my_church_options_color_fields();
		$cmsmasters_page_tabs = my_church_options_color_tabs();
	...
	case 'cmsmasters-settings' . '-element':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'my-church' . CMSMASTERS_THEME_STYLE . '_element';
		$cmsmasters_settings_page_title = esc_html__('Theme Elements Settings', 'my-church');
		$cmsmasters_page_sections = my_church_options_element_sections();
		$cmsmasters_page_fields = my_church_options_element_fields();
		$cmsmasters_page_tabs = my_church_options_element_tabs();
	...
	case 'cmsmasters-settings' . '-single':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'my-church' . CMSMASTERS_THEME_STYLE . '_single';
		$cmsmasters_settings_page_title = esc_html__('Theme Single Posts Settings', 'my-church');
		$cmsmasters_page_sections = my_church_options_single_sections();
		$cmsmasters_page_fields = my_church_options_single_fields();
		$cmsmasters_page_tabs = my_church_options_single_tabs();
	...
	case 'cmsmasters-settings' . '-demo':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'my-church' . CMSMASTERS_THEME_STYLE . '_demo';
		$cmsmasters_settings_page_title = esc_html__('Theme Settings Import/Export', 'my-church');
		$cmsmasters_page_sections = my_church_options_demo_sections();
		$cmsmasters_page_fields = my_church_options_demo_fields();
		$cmsmasters_page_tabs = my_church_options_demo_tabs();
...
function my_church_create_settings_field($args = array()) {
	$defaults = array( 
		'id' => 		'default_field', 
		'title' => 		esc_html__('Default Field', 'my-church'), 
		'desc' => 		esc_html__('This is a default description.', 'my-church'), 
		'std' => 		'', 
		'type' => 		'text', 
...
function my_church_register_settings() {
	$settings_output = my_church_get_settings();
	...
	register_setting($cmsmasters_option_name, $cmsmasters_option_name, 'my_church_validate_options');
	
	
	if (!empty($settings_output['cmsmasters_page_sections'])) {
		foreach ($settings_output['cmsmasters_page_sections'] as $id => $title) {
			add_settings_section($id, $title, 'my_church_section_fn', __FILE__);
		}
	}
	...
	if (!empty($settings_output['cmsmasters_page_fields'])) {
		foreach ($settings_output['cmsmasters_page_fields'] as $option) {
			my_church_create_settings_field($option);
		}
	}
...
function my_church_settings_scripts() {
	wp_enqueue_style('wp-jquery-ui-dialog');
	
	
	wp_enqueue_style('my-church-settings-css', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings.css', array(), '1.0.0', 'screen');
	
	
	if (is_rtl()) {
		wp_enqueue_style('my-church-settings-css-rtl', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings-rtl.css', array(), '1.0.0', 'screen');
	}
	...
	wp_enqueue_script('my-church-settings-js', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings.js', array('jquery', 'farbtastic'), '1.0.0', true);
	
	
	$theme_style_name = '';
	
	if (CMSMASTERS_THEME_STYLE_COMPATIBILITY) {
		foreach (my_church_all_theme_styles() as $theme_style) {
	...
	wp_localize_script('my-church-settings-js', 'cmsmasters_setting', array( 
		'palettes' => 						implode(',', cmsmasters_color_picker_palettes()), 
		'remove' => 						esc_attr__('Remove', 'my-church'), 
		'remove_sidebar' => 				esc_html__('Do you realy want to remove this sidebar?', 'my-church'), 
		'find' => 							esc_attr__('Find icons', 'my-church'), 
		'remove_icon' => 					esc_html__('Do you realy want to remove this social icon?', 'my-church'), 
		'theme_uri' => 						get_template_directory_uri(), 
		'shortname' => 						'my-church', 
		'theme_style_name' => 				$theme_style_name, 
		'done' => 							esc_html__('All theme settings were imported successfully.', 'my-church'), 
		'fail' => 							esc_html__("Error on page!!!\nPlease reload page and try again.", 'my-church'), 
		'done_reset_settings' => 			esc_html__('Theme style settings were reset to default.', 'my-church'), 
	...
	wp_enqueue_script('my-church-settings-js-toggle', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings-toggle.js', array('jquery'), '1.0.0', true);
	
	wp_localize_script('my-church-settings-js-toggle', 'cmsmasters_settings', array( 
		'shortname' => 	'my-church' 
	));
...
function my_church_add_menu() {
	...
	$add_menu_p( 
		esc_attr__('Theme Settings', 'my-church'), 
		esc_attr__('Theme Settings', 'my-church'), 
		'manage_options', 
		'cmsmasters-settings', 
		'my_church_settings_page_fn', 
		'' 
	);
	...
	$cmsmasters_settings_general = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme General Settings', 'my-church'), 
		esc_attr__('General', 'my-church'), 
		'manage_options', 
		'cmsmasters-settings', 
		'my_church_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_general, 'my_church_settings_scripts');
	
	
	$cmsmasters_settings_font = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Fonts Settings', 'my-church'), 
		esc_attr__('Fonts', 'my-church'), 
		'manage_options', 
		'cmsmasters-settings' . '-font', 
		'my_church_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_font, 'my_church_settings_scripts');
	
	
	$cmsmasters_settings_color = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Color Schemes', 'my-church'), 
		esc_attr__('Colors', 'my-church'), 
		'manage_options', 
		'cmsmasters-settings' . '-color', 
		'my_church_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_color, 'my_church_settings_scripts');
	
	
	$cmsmasters_settings_element = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Elements Settings', 'my-church'), 
		esc_attr__('Elements', 'my-church'), 
		'manage_options', 
		'cmsmasters-settings' . '-element', 
		'my_church_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_element, 'my_church_settings_scripts');
	
	
	$cmsmasters_settings_single = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Single Posts Settings', 'my-church'), 
		esc_attr__('Single Posts', 'my-church'), 
		'manage_options', 
		'cmsmasters-settings' . '-single', 
		'my_church_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_single, 'my_church_settings_scripts');
	
	
	$cmsmasters_settings_demo = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Settings Import/Export', 'my-church'), 
		esc_attr__('Import/Export', 'my-church'), 
		'manage_options', 
		'cmsmasters-settings' . '-demo', 
		'my_church_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_demo, 'my_church_settings_scripts');
}


function my_church_section_fn($desc) {
	$tab = my_church_get_the_tab();
...
function my_church_settings_page_fn() {
	$settings_output = my_church_get_settings();
	$current_tab = my_church_get_the_tab();
	
	echo '<div class="wrap">';
	
	my_church_settings_page_header();
	...
	if ($current_tab != 'wpml') {
		echo '<form action="options.php" method="post" class="cmsmasters_admin_page"' . (($current_tab == 'recaptcha') ? ' style="background-color:#fdffc6; padding:35px 0 15px 25px; margin:0;"' : '') . '>' . 
			'<p class="submit-top">' . 
				(($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') ? '<input name="submit-top" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'my-church') . '" />' : '') . 
			'</p>';
			...
			echo '<p class="submit">';
				...
				if ($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') {
					echo '<input name="submit" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'my-church') . '" />';
				} elseif ($current_tab == 'import') {
					echo '<input name="import" type="button" class="button button-primary button-large cmsmasters-demo-import" value="' . esc_attr__('Import Settings', 'my-church') . '" />';
				} elseif ($current_tab == 'theme_style') {
					echo '<input name="apply-theme-style" type="submit" class="button button-primary button-large cmsmasters-apply-theme-style" value="' . esc_attr__('Apply This Style', 'my-church') . '" />';
					echo '<input name="reset-settings" type="button" class="button button-large cmsmasters-reset-settings" value="' . esc_attr__('Reset Theme Style Settings to Default', 'my-church') . '" />';
				}
...
function my_church_form_field_fn($args = array()) {
	extract($args);
	...
	case 'select_theme_style':
		$theme_style_option = get_option('cmsmasters_my-church_theme_style');
		
		echo '<select id="' . $id . '" class="select' . $field_class . '" name="cmsmasters_my-church_theme_style">';
	...
	case 'upload':
		...
		echo '<div class="cmsmasters_upload_parent cmsmasters_select_parent">' . 
			'<input type="button" id="cmsmasters_upload_' . $id . '_button" class="cmsmasters_upload_button button button-large" value="' . esc_attr__('Choose Image', 'my-church') . '" data-title="' . esc_attr__('Choose Image', 'my-church') . '" data-button="' . esc_attr__('Insert Image', 'my-church') . '" data-id="cmsmasters-media-select-frame-' . $id . '" data-classes="media-frame cmsmasters-media-select-frame' . ((!isset($description)) ? ' cmsmasters-frame-no-description' : '') . ((!isset($caption)) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($align)) ? ' cmsmasters-frame-no-align' : '') . ((!isset($link)) ? ' cmsmasters-frame-no-link' : '') . ((!isset($size)) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $frame . '"' . (($frame == 'post') ? ' data-state="insert"' : '') . ' data-multiple="' . $multiple . '" />' . 
			'<div class="cmsmasters_upload"' . (($image != '') ? ' style="display:block;"' : '') . '>' . 
				'<img src="' . (($image != '') ? $image : '') . '" class="cmsmasters_preview_image" alt="" />' . 
				'<a href="#" class="cmsmasters_upload_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'my-church') . '"></a>' . 
			'</div>' . 
	...
	case 'typorgaphy':
		...
		if ($system_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (my_church_system_fonts_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_system_font'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_system_font">' . esc_html__('System Font', 'my-church') . '</label>' . 
			'</div>';
		}
		...
		if ($google_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_google_font">' . esc_html__('Google Font', 'my-church') . '</label>' . 
			'</div>';
		}
		...
		if ($font_size) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_size_number">' . esc_html__('Font Size', 'my-church') . ' <em>(' . esc_html__('in pixels', 'my-church') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($line_height) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_line_height_number">' . esc_html__('Line Height', 'my-church') . ' <em>(' . esc_html__('in pixels', 'my-church') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($font_weight) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_weight">' . esc_html__('Font Weight', 'my-church') . '</label>' . 
			'</div>';
		}
		...
		if ($font_style) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_style">' . esc_html__('Font Style', 'my-church') . '</label>' . 
			'</div>';
		}
		...
		if ($text_transform) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_transform">' . esc_html__('Text Transform', 'my-church') . '</label>' . 
			'</div>';
		}
		...
		if ($text_decoration) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (my_church_text_decoration_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_text_decoration'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_decoration">' . esc_html__('Text Decoration', 'my-church') . '</label>' . 
			'</div>';
		}
	...
	case 'sidebar':
		...
		'<div class="sidebar_management">' . 
			'<p>' . 
				...
				'<input class="button" type="button" id="add_sidebar" value="' . esc_attr__('Add Sidebar', 'my-church') . '" />' . 
			'</p>' . 
			'<div></div>' . 
			'<ul>';
	...
	case 'social':
		...
		'<div class="icon_management">' . 
			'<p>' . 
				...
				'<input id="' . $id . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'my-church') . '" />' . 
				'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Cancel changes', 'my-church') . '"></a>' . 
			'</p>' . 
			'<span class="cl"><br /></span>' . 
			'<span class="icon_upload_link" style="display:none;">' . 
				'<label for="new_icon_color">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Color', 'my-church') . '</span>' . 
				...
				'<label for="new_icon_hover">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Hover Color', 'my-church') . '<br /><em>' . esc_html__('if not selected - Icon Color will be used', 'my-church') . '</em></span>' . 
				...
				'<label for="new_icon_link">' . 
					...
					esc_html__('Icon Link', 'my-church') . 
				'</label>' . 
				'<label for="new_icon_title">' . 
					'<input class="all-options" type="text" id="new_icon_title" /> ' . 
					esc_html__('Icon Title', 'my-church') . 
				'</label>' . 
				'<label for="new_icon_target">' . 
					'<input type="checkbox" id="new_icon_target" value="true" /> ' . 
					esc_html__('Open link in a new tab/window?', 'my-church') . 
				'</label>' . 
			'</span>' . 
			'<span class="cl"></span>' . 
			'<input class="button button-primary" type="button" id="add_icon" value="' . esc_attr__('Add Icon', 'my-church') . '" />' . 
			'<input class="button button-primary" type="button" id="edit_icon" value="' . esc_attr__('Save Icon', 'my-church') . '" />' . 
			'<ul>';
			...
			if (isset($options[$id]) && is_array($options[$id])) {
				foreach($options[$id] as $icon) {
					...
					echo '<li>' . 
						...
						'</div>' . 
						'<a href="#" class="icon_del admin-icon-remove" title="' . esc_attr__('Remove', 'my-church') . '"></a> ' . 
...
function my_church_validate_options($input) {
	$valid_input = array();
	...
	foreach ($options as $option) {
		...
		case 'text':
			...
			case 'numeric':
				...
				$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Expecting a Numeric value!', 'my-church');
				
				if (is_numeric($input[$option['id']]) == false) {
					add_settings_error(
						$option['id'],
						'my-church' . '_txt_numeric_error',
						esc_html__('Expecting a Numeric value! Please fix.', 'my-church'),
						'error'
					);
				}
			...
			case 'multinumeric':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : esc_html__('Expecting comma separated numeric values', 'my-church');
				} else {
				...
				if ($input[$option['id']] != '' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
					add_settings_error(
						$option['id'],
						'my-church' . '_txt_multinumeric_error',
						esc_html__('Expecting comma separated numeric values! Please fix.','my-church'),
						'error'
					);
				}
			...
			case 'email':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (is_email($input[$option['id']]) !== false) ? $input[$option['id']] : esc_html__('Invalid email! Please re-enter!', 'my-church');
				} elseif ($input[$option['id']] == '') {
					$valid_input[$option['id']] = esc_html__('This setting field cannot be empty! Please enter a valid email address.', 'my-church');
				}
				
				if (is_email($input[$option['id']]) == false || $input[$option['id']] == '') {
					add_settings_error(
						$option['id'],
						'my-church' . '_txt_email_error',
						esc_html__('Please enter a valid email address.', 'my-church'),
						'error'
					);
				}
		...
		case 'number':
			$input[$option['id']] = trim($input[$option['id']]);
			$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Number!', 'my-church');
...
function my_church_show_msg($message, $msgclass = 'info') {
	echo '<div id="message" class="' . $msgclass . '">' . $message . '</div>';
}


function my_church_admin_msgs() {
	global $pagenow;
...
function my_church_add_global_options($reset = false) {
	...
	foreach (my_church_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, array( 
			'cmsmasters_options_' . 'my-church' . CMSMASTERS_THEME_STYLE . '_color_' . $key, 
			my_church_options_color_fields($key) 
		));
	}
		...
		if ($cmsmasters_option_name[1] !== '') {
			foreach ($cmsmasters_option_name[1] as $selected_option) {
				if ( 
					...
					$selected_option['id'] !== 'my-church' . '_social_icons' 
				) {
...
function my_church_get_global_options() {
	...
	foreach (my_church_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, 'cmsmasters_options_' . 'my-church' . CMSMASTERS_THEME_STYLE . '_color_' . $key);
	}
